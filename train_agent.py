...
def train_agent(restore_prior_from='data/Prior.ckpt',
    ...
    for step in range(n_steps):
        ...
        for i in range(10):
            try:
                ...
                print("Outputting nonsense molecules and too many duplicates.")
    ...
    torch.save(Agent.rnn.state_dict(), os.path.join(save_dir, save_to))
...
if __name__ == "__main__":
    train_agent(restore_prior_from='models/Prior_focused.ckpt',                                                     
                  restore_agent_from='models/Prior_focused.ckpt',                                                     
                  voc_file = '../vocabularies/Voc_joined',                                                            
                  save_to= 'Agent.ckpt',                                                                              
                  scoring_function='pIC50_pred',
                  scoring_function_kwargs={'path_to_model':'qsar_models/subset/pic50_model.hdf5', 'path_to_scaler':'', 'pic50_term':7},
                  save_dir='rl/subset',
                  n_steps = 3000, learning_rate=0.0001, experience_replay=0)
